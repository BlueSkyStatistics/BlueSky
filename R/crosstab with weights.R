                          ...
                          dnn = NULL,index)
{
  ...
  #Added by Aaron 06/25/2023 set drop.unused.levels = FALSE so that mcnemar test would run
  #Added by Aaron 06/25/2023 we still suppress all rows with 0 counts in the display but these 0 counts are used to compute the statistics
  uatab <-xtabs(formula=uaform,data=uadatasets$lst[[index]],drop.unused.levels = FALSE)
...
uapreptableforstats <-function(t,uatempindex)
{
  ...
  if (length(class(orit))==1)
  {
    #19Jan2017 Added "integer" condition in following.
    #without that 'if' was failing and further the crosstab was failing.
    #From Cars.csv put 'origin' in 1st, 'year' in 2nd and 'cylinder' in 3rd box, you will see an error, if
    # 'integer' condition is not present in following 'if'
    if(class(orit)=="numeric" || class(orit)=="integer") orit =matrix(orit,nrow=1)
  }
  ...
  #The code below creates a matrix that will be used to compute the statistics
  ##Added code below  06/24/2023
  uanoofcols=ncol(orit)
  j=1
  uatforstatsMcnemar =matrix(nrow =nrow(orit),ncol=0)	
  for (i in 1:uanoofcols)
  {
    if (sum(orit[,i]) ==0)
    {
    ...
    {
      uatforstatsMcnemar =cbind(uatforstatsMcnemar,orit[,i])
    }	
  }
  
  BSkyFunctionWrapUp()
  return(list(t=t,uatforstats=uatforstats,tpercents=tpercents, uatforstatsMcnemar=uatforstatsMcnemar))			
}
                          ...
                          format = NULL, dnn = NULL, factorvals=NULL,noNonEmptyTables,storeOriLevelOfFirstVarIfOneByOneTable, ...) 
{
  ...
  numberOfRows =nrow(uaretstructure$uatforstatsMcnemar)
  numberOfCols =ncol(uaretstructure$uatforstatsMcnemar)
  ...
  if (any(dim(uaretstructure$uatforstats) < 2)) 
  {
    ...
    # 06/24/2023 commented line below
    prop.chisq <- chisq <-  fisher <- asresid <-resid<-sresid<- FALSE
    # 06/24/2023 commented line below
    #prop.chisq <- chisq <- asresid <-resid<-sresid<- FALSE
  ...
  if (mcnemar) 
  {
    ...
    #if (all(dim(uaretstructure$uatforstats) == 2)) 
    ua.McNc <- mcnemar.test(uaretstructure$uatforstatsMcnemar, correct = mcnemarCorrect)
  }
  if (asresid) 
  {
