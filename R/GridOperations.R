...
#																											
#############################################################################################################
BSkyAddVarRowOld <- function(newcolname, datagridcolval, newcolindex=0, dataSetNameOrIndex)
{
	BSkyFunctionInit()
	BSkySetCurrentDatasetName(dataSetNameOrIndex)
	
	BSkyErrMsg = paste("BSkyAddVarRow: Error in Add Var Row : ", "DataSetName :", dataSetNameOrIndex," ", "Variable Name List :", paste(newcolname, collapse = ","),sep="")
	BSkyWarnMsg = paste("BSkyAddVarRow: Warning in Add Var Row : ", "DataSetName :", dataSetNameOrIndex," ", "Variable Name List :", paste(newcolname, collapse = ","),sep="")
	BSkyStoreApplicationWarnErrMsg(BSkyWarnMsg, BSkyErrMsg)
	...
	tryCatch(
		{
			...
			withCallingHandlers(
			{
				...
				##"Role"  "Columns"  "Decimals"  "Width"  "Align"  "Label"  "Name"  "Split"  "Type" "Levels"  "Missing"  "Measure"  
				props<- c("Name","Type","Label","Levels","Missing","Align","Measure","Split","Width","Decimals","Columns","Role")
				pval <- list(newcolname, "double", newcolname, "", "none", "Left", "Scale", FALSE, 4,0,8, "Input")#new col defaults. Match it to CS
				#pval <- list(newcolname, "character", newcolname, "", "none", "Left", "Scale", FALSE, 4,0,8, "Input")#new col defaults. Match it to CS
				#in above "character" will put factor value=1. if its "" then it will show string in table but in that case 'Type' is lost.
				#You cant access properties of this col, whose Type is lost. So "character" makes more sense here
				...
				{	
					#cat("Dataset Found!")
					##colIndex<-BSkyValidateColumn(DataSetIndex, newcolname)
					colIndex <- BSkyValidateColumn(datasetname, newcolname)		
					#New col must not have same name as existing ones
					if(colIndex < 1)#col not found. Means col with this name does not already exists and we are safe to create it.
					{
						#cat("\nCol not found...", newcolname)

						###creating missing value attribute, which is dataset level att.
						newcolmisatt <- eval(parse(text=paste(newcolname,'<-list(',newcolname,'=list(type="none", value=""))')))
						#cat("\nMissing Attr: \n")
						#print(newcolmisatt)
						#Making a backup copy of "variable.labels"
						newvarlbls <- c() # storing variable.lables in same order as the col names. This attribute is detailed col name rather than short name.
						varlabels <- eval(parse(text=paste('attr(',datasetname,',"variable.labels")' )))
						misvals <- eval(parse(text=paste('attr(',datasetname,',"misvals")' )))##Get misval for all cols
						oldcolscount <- eval(parse(text=paste('ncol(',datasetname,')' )))
						cols <- oldcolscount + 1
						#cat("\nCol idx:")
						#cat(cols)
						#cat("\nVarLBLs: \n")
						...
						# cat("\n")
						#for var labels
						vlbls <- eval(parse(text=paste('c(',newcolname,' = newcolname)' )))
						#cat("\nVarLBL: \n")
						#print(vlbls)
						#cat("\nBacking up Dataset Level Attributes")
						BkupObj <- BSkyDatasetLevelAttributeBackup(datasetname)
						#cat("\nBacked up attributes:\n")
						#print(BkupObj)
						
						# add to the dataset at given index. Index should exists between 1 and old_max_col+1, inclusive.
						if( (newcolindex > 0) || (newcolindex <= cols) )
						{
							#cat("\nDS colnames1:")
							#print(names(Dataset1))
							n <- newcolindex # new col location# cols #
							#cat("\nNew col idx:")
							#cat(n)
							max <- oldcolscount
							eval(parse(text=paste('oldcolnames <- names(',datasetname,')', sep='')))#. <<- to <-
							#cat("\n Old col names:")
							#cat(oldcolnames)
							#cat("\nNow creating new col\n")
							newcol <- eval(parse(text=paste('c(rep( c(0), nrow(',datasetname,')))', sep='' ))) # " " was replace by 0
							#newcol <- eval(parse(text=paste('c(rep( c(NA), nrow(',datasetname,')))', sep='' ))) # " " was replace by 0
							if(n == (max+1)) #if adding as a last col
							{
								#cat("\nInsert as last col\n")
								newcolnames = c( oldcolnames[1:(n-1)], newcolname)
								newvarlbls = c( varlabels[1:(n-1)], vlbls)
								#cat("\nNow adding col\n")
								#print(newcol)
								eval(parse(text=paste(datasetname,' <- cbind(',datasetname,', newcol, stringsAsFactors=FALSE)',sep='')))# insert as last column#. <<- to <-
								#cat("\nDS colnames2:")
								#print(names(Dataset1))
							}
							else if(n == 1) #if adding as a first col
							{
								cat("\nInsert as first col\n")
								...
								#cat("\nNow adding col\n")
								#print(newcol)
								eval(parse(text=paste(datasetname,' <- cbind(',datasetname,',newcol, stringsAsFactors=FALSE)[, c((',max,'+1), 1:',max,' )]',sep='')))# insert in first#. <<- to <-
								#cat("\nDS colnames2:")
								#print(names(Dataset1))
							}
							else #inserting anywhere before last. 
							{
								#cat("\nInsert somewhere in between first and last\n")
								newcolnames = c( oldcolnames[1:(n-1)], newcolname, oldcolnames[n:max] ) 
								newvarlbls = c( varlabels[1:(n-1)], vlbls, varlabels[n:max] ) 
								#cat("\nNow adding new col somewhere in between first and last col\n")
								#print(newcol)
								eval(parse(text=paste(datasetname,' <- cbind(',datasetname,', newcol, stringsAsFactors=FALSE)[, c(1:(',n,'-1), (',max,'+1), ',n,':',max,' )]',sep='')))# insert in middle#. <<- to <-
								#cat("\nDS colnames2:")
								#print(names(Dataset1))
							}
							eval(parse(text=paste('names(',datasetname,') <- c(newcolnames)', sep='')))#. <<- to <-

							## setting variables.labels
							#cat("\nAlready Set var lbls")
							#eval(parse(text=paste('attr(',datasetname,',"variable.labels") <<-',newvarlbls )))

							#cat("\nSet missings")
							#eval(parse(text=paste('attr(',datasetname,',"misvals") <<- c(attr(',datasetname,',"misvals")[1:',(cols-1),'], ',newcolmisatt,')',sep='' )))	
							#cat("\nnew col added.\n")		
						}
						...
						for( prop in props)
						{
							#cat("\n",prop,":")
							#print(pval[[i]])
							UAsetColProperties(dataSetNameOrIndex=datasetname, colNameOrIndex=newcolname, propertyName=prop, propertyValue=pval[[i]], mistype="none",newOrder=c())
							#cat("Set")
							i=i+1
						}
						#cat("\nRestore col props and datasets attributes\n")
						## setting variables.labels
						#cat("\nSet var lbls")
						eval(parse(text=paste('attr(',datasetname,',"variable.labels") <- newvarlbls' )))#. <<- to <-

						#cat("\nSet missings")
						eval(parse(text=paste('attr(',datasetname,',"misvals") <- c(attr(',datasetname,',"misvals")[1:',(cols-1),'], ',newcolmisatt,')',sep='' )))#. <<- to <-
						#cat("\n Milestone 5")
						#print(lost)		
						### putting a backup copy in place ###because we have lost these properties/attributes
						#cat("\nRestoring Dataset Level Attributes")
						BSkyDatasetLevelAttributeRestore(datasetname, BkupObj)
						#cat("\nDataset Attributes and Col properties restored.\n")
					}#col not found
					else
					{
						# if col already exists then we need to cancel the col add operation from UI grid.
						# cat("\nCol with that name already exists...\n")
						BSkyErrMsg =paste("BSkyAddVarRow: Col with that name already exists..."," Dataset Name:", dataSetNameOrIndex)
				...
				{
					# cat("\nDataset not found!...\n")
					BSkyErrMsg =paste("BSkyAddVarRow: Dataset not found!..."," Dataset Name:", dataSetNameOrIndex)
		...
		if(BSkyLocalErrorFound() == TRUE)
    	{
			...
			#cat("Error caught in BSkyAddVarRow \n")
			BSkyLocalErrorFlagsReset() #if needed
    	}
		#cat("\nWARNING:: top level Add Var Row function\n")
		if(BSkyLocalWarningFound() == TRUE)
    	{
    		# if anything needs to be checked or print etc
			# All Bsky functions continue from the point where the warning occured
			...
			#cat("\nWarning caught in BSkyAddVarRow begin \n")
			BSkyLocalWarningFlagsReset() #if needed to continue without returning back to the top level function 
			#cat("\nWarning caught in BSkyAddVarRow end \n")
    	}
		# cat("\nBefore wrapup")
		# print("Call Stack:")
		...
		# cat("\nUberFunctName")
		# print(uadatasets.sk$uaTempUberFunctionName)		
		BSkyFunctionWrapUp()
		# cat("\nAfter wrapup")
		# print("Call Stack:")
		# print(uadatasets.sk$callStack )
		...
		# print(uadatasets.sk$uaTempUberFunctionName)
		# #print(BSkyReturnStructure())
		# cat("\nReturning return structure from this top level Add Var Row function\n")
		return(BSkyReturnStructure2())
}

# BSkyAddVarRow <- function(newcolname, rdatatype, datagridcolval, newcolindex=0, dataSetNameOrIndex)
# {
	# BSkyFunctionInit()
	# BSkySetCurrentDatasetName(dataSetNameOrIndex)
	...
	# tryCatch(
		# {
			...
			# {
				...
				# {
				# pval <- list(newcolname, rdatatype,  newcolname, "", "none", "Left", "Nominal", FALSE, 4,0,8,"Input")#new col defaults. Match it to CS
				# }
				# else
				# {
				# pval <- list(newcolname, rdatatype,  newcolname, "", "none", "Left", "Scale", FALSE, 4,0,8,"Input")#new col defaults. Match it to CS
				# }
				# #in above "character" will put factor value=1. if its "" then it will show string in table but in that case 'Type' is lost.
				# #You cant access properties of this col, whose Type is lost. So "character" makes more sense here
				...
				# {	
					...
					# {
						...
						# {
							...
							# else # for "character"
							# {
								# strasfact=TRUE #appears like its not having any impact. It should though
								# newcol <- eval(parse(text=paste('c(rep( c(NA), nrow(',datasetname,')))', sep='' ))) # " " was replace by 0
							# }
							# if(n == (max+1)) #if adding as a last col
							# {
								...
								# #cat("\nNow adding col\n")
								# #print(newcol)
								# eval(parse(text=paste(datasetname,' <- cbind(',datasetname,', newcol, stringsAsFactors=',strasfact,')',sep='')))# insert as last column. <<- to <-
								# #cat("\nDS colnames2:")
								# #print(names(Dataset1))
							# }
							# else if(n == 1) #if adding as a first col
							# {
								...
								# #cat("\nNow adding col\n")
								# #print(newcol)
								# eval(parse(text=paste(datasetname,' <- cbind(',datasetname,',newcol, stringsAsFactors=',strasfact,')[, c((',max,'+1), 1:',max,' )]',sep='')))# insert in first.. <<- to <-
								# #cat("\nDS colnames2:")
								# #print(names(Dataset1))
							# }
							# else #inserting anywhere before last. 
							# {
								...
								# #cat("\nNow adding new col somewhere in between first and last col\n")
								# #print(newcol)
								# eval(parse(text=paste(datasetname,' <- cbind(',datasetname,', newcol, stringsAsFactors=',strasfact,')[, c(1:(',n,'-1), (',max,'+1), ',n,':',max,' )]',sep='')))# insert in middle. <<- to <-
								# #cat("\nDS colnames2:")
								# #print(names(Dataset1))
							# }
							# eval(parse(text=paste('names(',datasetname,') <- c(newcolnames)', sep='')))#. <<- to <-
