...
###################################################################################################################
UAreadRObj.old <- function(RObjfileName, datasetname, replace=FALSE) 
{
	BSkyFunctionInit()
	...
	BSkyStoreApplicationWarnErrMsg(BSkyWarnMsg, BSkyErrMsg)
			...
			#print(Sys.time())			
					...
					load(file = RObjfileName)#, uadatasets$lst[Newindex])
			#cat("After actual function call load():",datasetname)
			#print(Sys.time())						
					
					# uadatasets$name[Newindex] <- fileName or optionalName
					# cat("\nUAObj: obj")
					# print(is.null(UAObj$obj))
					# cat("\nUAObj info: ")
					# print(is.null(UAObj$info))	
					...
					if(!is.null(bskysig) && bskysig$ver==1.2 && bskysig$name=="BSky")
					{
						# cat("\nInside if..\n")
						# Now add new dataset.
						# uadatasets$lst <- c(uadatasets$lst, list(UAObj$obj))
						eval( parse(text=paste( '.GlobalEnv$',datasetname,' <- UAObj$obj' , sep=''   ))) ## Working
						#eval( parse(text=paste( 'x <<- UAObj$obj' , sep=''   )))## used with lapply
						#eval(parse(text=paste(datasetname,' <<- as.data.frame( lapply (x, function(y) {if(class(y) == "character") y = factor(y) else y} ) )' )))
						
						#finding if vector contains a string
						#"POSIXct" %in% class(Dataset1$bdate)
						#eval(parse(text=paste(datasetname,' <<- as.data.frame( lapply (x, function(y) {if("POSIXct" %in% class(y)) y = as.character(y) else y} ) )' )))
			#cat("Eval PArse",datasetname)
			#print(Sys.time())	
						...
						#cat("\nLoaded ROBJ dataset :", datasetname)						
			...
			#cat("Attributes:",datasetname)
			#print(Sys.time())							

						#Creating extra attributes at column level
						UAcreateExtraAttributes(datasetname, "RDATA")
			#cat("Extra Attrbutes created:",datasetname)
			#print(Sys.time())							
					}
					else
					{
						BSkyErrMsg =paste("UAreadRObj: Not a Blue Sky file version."," File Name:", RObjfileName)
						warning("UAreadRObj: Not a Blue Sky file version.");
...
###################################################################################################################
UAwriteRObj.old <- function(RObjfileName,dataSetNameOrIndex) ##  index of dataset and file
{
	BSkyFunctionInit()
...
datasetname <- BSkyValidateDataset(dataSetNameOrIndex)
			...
			{		
				#Bsky signature
				objInfo <- list(ver=1.2, name="BSky")
				#cat("Align:");
				aln=eval(parse(text=paste('attr(',datasetname,'[,1],"align")',sep='')))
				#cat(aln)
				#save(rObjects, objInfo, file = fileName)
				#UAObj <- list(obj=rObjects, name=objname, info=objInfo)
				UAObj <- eval(parse(text=paste('list(obj=',datasetname,', info=objInfo)')))
				save(UAObj, file = RObjfileName)#
