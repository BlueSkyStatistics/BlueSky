old.BSky.LoadRefresh.Dataframe <- function(dframe, datasetname)
{
	BSkyFunctionInit()
	...
	if(eval( parse(text=paste('exists("dframe")',sep=''))))#exists(dframe)) 
	{
		...
		{
			dsname <- BSkyValidateDataset(datasetname)
			#if dataset index is valid
			if(!is.null(dsname))##Just refresh dataset.
			{
				#eval( parse(text=paste(dsname,' <<- dframe',sep='')))
			}
			else ## Dataset not already present in uadatasets$name
			{
				#cat("\n DF's DS name:", datasetname)
				#eval( parse(text=paste(datasetname,' <<- dframe',sep='')))
				uadatasets$name <- c(uadatasets$name, datasetname)
				#cat("\nLoaded dataframe dataset :", datasetname)
				#cat("\n Creating Attrs for DS:", datasetname)
				eval(parse(text=paste('attributes(',datasetname,') <<- c(attributes(',datasetname,'), list(split = c(FALSE), splitcolumnnames = c(), splitcolumnindex = c(), filetype = c("CSV"), slice = c(FALSE), align = c("Left"), maxfactor= c(bskymaxfactors)))', sep='')))							
				UAcreateExtraAttributes(datasetname)
				#cat("\n Finished Creating Attrs for DS:", datasetname)
			}
		}
		else
		{
