...
BSkyLoadDATinDataFrame <- function(DATfilename, datasetname, replace=FALSE, Header=csvHeader, sepCh='\t', deciCh='.')
{
	...
	if((replace == TRUE) || (replace == FALSE && curidx == -1))
	{
		...
		#df <- read_sas("D:/BlueSky Statistics/Misc/Data_Files/DAT/ctcodes-procedures.sas7bdat")
		eval( parse(text=paste('x <<-  read.delim(file=\'',DATfilename,'\', header =', Header,', sep = \'',sepCh,'\', dec =\'',deciCh,'\' )',sep='' ))) # use with lapply
